...
// Function to check if the response status is handled correctly (response.ok)
function checkResponseStatusHandling() {
    let result = 'Pass';
    let feedback = [];
    ...
    // Check if response.ok is used to handle status codes
    if (!studentCode.includes('response.ok')) {
        result = 'Fail';
        feedback.push("You must use response.ok to handle successful status codes (200-299).");
    }

    // Detailed logging of the check
    console.log(`\x1b[33mChecking response status code handling\x1b[0m`);

    return new TestCaseResultDto(
        'ResponseStatusHandling',
        'functional',
        1,
        result === 'Pass' ? 1 : 0,
        result,
...
function gradeAssignment() {
    ...
    testResults.testCaseResults[GUID + '-response-status'] = checkResponseStatusHandling();
